...
Market* Market::make_market(std::string symbol, std::string venue)
{
    ...
    else if(venue=="SZ") return new ShenZhenMarket(symbol);
...
bool Market::IsOpen()
{
    return (time_ >= add_minutes(0, mo_)) and (add_minutes(0, mc_) >= time_);
}
...
double Market::tick_size(double price)
{
    ...
    //在pts_的key中寻找刚好大于price的key，然后往前取一个，相当于map.key版本的floor
...
// Intermediate:
ChineseMarket::ChineseMarket(std::string symbol, Currency c,long mo, long mc):
...
/*
 * 下面的pts_其实就是不同price级别下每个tick对应的delta price的大小
 *
 */
    Market(symbol, c, mo, mc, {
            {100000., 100.},
